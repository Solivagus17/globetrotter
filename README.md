...
The database is built on PostgreSQL via Supabase. Core tables include:
...
activities (
    ...
    name TEXT NOT NULL,
    category TEXT, -- 'sightseeing', 'food', 'stay', 'adventure', 'transport'
...
- Optional: API Key from [Groq Cloud](https://console.groq.com/) for Voyage AI

---

### 1. Database Setup (Supabase)
1. Create a new project in your Supabase Dashboard.
2. Open the **SQL Editor** in Supabase, paste the contents of `schema.sql`, and click **Run**.
3. Under **Authentication -> Settings**, ensure Email provider is enabled. (Optionally disable "Confirm email" for instant test signups).
4. Copy your **Project URL**, **Anon Public Key**, and **Service Role Key** from **Settings -> API**.
...
GROQ_API_KEY=your-groq-api-key   # Optional for Voyage AI LLM
PORT=5000
```
...
| `SUPABASE_SERVICE_KEY` | `backend/.env` | Supabase service role secret key |
| `GROQ_API_KEY` | `backend/.env` | Groq Cloud API key for Voyage AI LLM |
| `VITE_SUPABASE_URL` | `frontend/.env` | Public Supabase project URL |
| `VITE_SUPABASE_ANON_KEY` | `frontend/.env` | Public Supabase anon key |
| `VITE_API_URL` | `frontend/.env` | URL of the Flask backend (`http://localhost:5000`) |
